    ...
    public Result checkDone(@RequestBody Integer[] ids) throws MessagingException {
        ...
        if (commentService.checkDone(ids)){
            for (Integer commentId:ids){
                // 根据博主审核的评论id查询评论
                com.app.blog.entity.Comment comment = commentService.selectByPrimaryKey(commentId);
                // 如果不等于0(子评论)
                if(comment.getIsReply()!=0){
                    // 根据子评论的isReply查询父评论邮箱
                	com.app.blog.entity.Comment reply = commentService.selectByPrimaryKey(comment.getIsReply());
